...
- **Vampire covens** spawn in dark forests at night via `world/VampireCovenSpawner.java` (radial `LevelTickEvent.Post` scan, NOT `ChunkEvent.Load` — the latter causes deadlocks). Config in `config/ModServerConfig.java` (SERVER type, per-world TOML).
